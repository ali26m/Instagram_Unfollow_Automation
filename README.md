...
2. Create a `usernames.txt` file in the same directory and list the usernames you want to unfollow (one per line).
   ...
   - After downloading the zip file and exporting the files go to [comparetwolists.com](https://comparetwolists.com/) upload `following.html` into List A and `followers_1.html` into List B, then check the Instagram followers checkbox and hit compare lists.
